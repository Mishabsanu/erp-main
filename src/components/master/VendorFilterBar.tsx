...
import { VendorFilter } from '@/lib/types';
...
interface VendorFilterBarProps {
  onStatusChange: (status: VendorFilter['status']) => void;
  onClearFilters: () => void;
  initialStatus?: VendorFilter['status'];
}
...
export const VendorFilterBar: React.FC<VendorFilterBarProps> = ({
  ...
  useEffect(() => {
    onStatusChange(selectedStatus === '' ? undefined : (selectedStatus as VendorFilter['status']));
